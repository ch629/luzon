...
class FiniteStateMachine<T, O>(private var states: List<State<T, O>>, updateEpsilons: Boolean = true) {
    ...
    companion object {
        fun <O : Any> fromRegex(str: String, out: O? = null) = FiniteStateMachine(listOf(RegexScanner<O>(str).toFSM(out)))

        fun <T, O> merge(vararg machines: FiniteStateMachine<T, O>): FiniteStateMachine<T, O> {
            val allInitialStates = mutableListOf<State<T, O>>()

            machines.forEach { allInitialStates.addAll(it.initialStates) }

            return FiniteStateMachine(allInitialStates, false)
        }
    }

    fun merge(other: FiniteStateMachine<T, O>) =
        FiniteStateMachine(initialStates + other.initialStates, false)
    ...
    fun copyOriginal() = FiniteStateMachine(initialStates, false)
