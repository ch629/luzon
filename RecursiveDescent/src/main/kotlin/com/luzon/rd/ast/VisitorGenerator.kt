...
object VisitorGenerator {
    fun generate() {
        ...
        acceptSb.appendln("fun <T> ASTNode.accept(visitor: ASTNodeVisitor<T>) = when (this) {") // TODO: Make this = when (this) ?

        lines.forEach {
            val trimmed = it.trim()

            visitorSb.indent().appendln("fun visit(node: $trimmed): T")
            acceptSb.indent().appendln("is $trimmed -> visitor.visit(this)")
        }
